...
export default function MixedPickemRecap({
  leagueId,
  season,
  viewerFantasyTeamId,
  enabledSports,
      ...
      async () => {
        const [
          ...
          await Promise.all([
            ...
            supabase.rpc(
              "get_pickem_unified_trophy_case",
              {
                p_league_id:
                  leagueId,
              }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`mixed-pickem-recap-${leagueId}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "pickem_weekly_results", filter: `league_id=eq.${leagueId}` }, refresh)
      .on("postgres_changes", { event: "*", schema: "public", table: "pickem_badge_awards", filter: `league_id=eq.${leagueId}` }, refresh)
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "nhl_pickem_awards", filter: `league_id=eq.${leagueId}` }, refresh)
      .subscribe();
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Official combined weekly winners and records across every enabled sport. CFB, NFL and NHL selections are finalized as one G365 Pick&apos;em card, with one weekly result feeding the season recap.
        </p>
      </section>
      ...
      0 ? (
        <EmptyState
          text="Recaps unlock after the first combined Pick'em week is officially finalized."
        />
      ) : (
        <>
          <section
          ...
          >
            ...
            0 ? (
              ...
              <EmptyState
                text="Trophy cases will appear after permanent G365 Pick’em awards are earned."
              />
            )}
          </section>
        </>
          ...
          {badges.map(
            (
              ...
              >
                ...
                >
                  {
                    badge.details?.sourceSport ===
                    "nhl"
                      ? `NHL · ${badge.badge_name}`
                      : badge.badge_name
                  }
                </span>
