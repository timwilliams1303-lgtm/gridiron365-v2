...
export async function getTraditionalPlayoffMatchupDetailData(
  supabase:
    SupabaseClient,
  leagueId: string,
  matchupId: number,
  ...
  if (
    ...
    throw new Error(
      `Could not load playoff matchup: ${playoffMatchupError.message}`
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Traditional playoff matchup was not found."
    );
  }
  ...
  if (
    ...
    throw new Error(
      "This playoff matchup is not ready yet."
    );
  }
  ...
  const {
    ...
    await supabase.rpc(
      "refresh_traditional_playoff_matchup",
      {
        p_playoff_matchup_id:
          matchupId,
      }
    );
  ...
  if (
    ...
    throw new Error(
      `Could not refresh playoff matchup: ${refreshError.message}`
    );
  }


  /*
   * Reload after refresh and normalize playoff_week to the
   * same internal week shape used by the regular matchup
   * detail pipeline.
   */

  const {
    data:
      refreshedPlayoffData,
  ...
  if (
    ...
    throw new Error(
      `Could not reload playoff matchup: ${refreshedError.message}`
    );
  }
  ...
  const refreshed:
    MatchupRow = {
      id:
        refreshedPlayoffData.id,

      league_id:
        refreshedPlayoffData.league_id,

      season:
        refreshedPlayoffData.season,

      week:
        refreshedPlayoffData.playoff_week,

      home_fantasy_team_id:
        refreshedPlayoffData.home_fantasy_team_id,

      away_fantasy_team_id:
        refreshedPlayoffData.away_fantasy_team_id,

      home_points:
        refreshedPlayoffData.home_points,

      away_points:
        refreshedPlayoffData.away_points,

      is_live:
        refreshedPlayoffData.is_live,

      is_final:
        refreshedPlayoffData.is_final,

      winner_fantasy_team_id:
        refreshedPlayoffData.winner_fantasy_team_id,

      tied:
        refreshedPlayoffData.tied,
    };
