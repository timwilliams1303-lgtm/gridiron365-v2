...
import {
  getSeasonLongMatchupDetailData,
  type MatchupDetailGameContext,
  type MatchupDetailPlayer,
  type MatchupDetailTeam,
  type SeasonLongMatchupDetailData,
} from "@/lib/season-long/matchup-detail.service";
...
export default async function SeasonLongMatchupDetailPage({
  params,
}: PageProps) {
  const {
    leagueId,
  ...
  // Membership is authorized above. The matchup is league-shared data,
  // so use the server-only admin client to prevent RLS from hiding the
  // opponent/matchup and incorrectly turning a valid matchup into a 404.
  const supabase =
    createSupabaseAdminClient();
  ...
  try {
    data =
      await getSeasonLongMatchupDetailData(
        supabase,
        leagueId,
        matchupId,
        access.fantasyTeam
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Link
            href={`/league/${leagueId}/season-long/matchups?week=${data.week}`}
            style={
              styles.backLink
            }
          >
          ...
          >
            {(data.home.isMyTeam ||
              data.away.isMyTeam) ? (
              <Link
                href={`/league/${leagueId}/entry?edit=1`}
                style={
                  styles.editLineupLink
                }
              >
                EDIT MY LINEUP
              </Link>
...
const styles = {
  ...
  editLineupLink: {
    ...
    border:
      "1px solid rgba(255,113,20,.42)",
    borderRadius: "8px",
    background:
      "linear-gradient(135deg,rgba(196,20,20,.92),rgba(255,91,0,.92))",
    color: "#ffffff",
    fontSize: "9px",
    fontWeight: 950,
    letterSpacing: ".05em",
