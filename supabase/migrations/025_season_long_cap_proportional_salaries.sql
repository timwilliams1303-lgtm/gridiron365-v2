...
begin
  ...
  -- =========================================================
  -- SCALE PLAYER PRICES TO THIS LEAGUE'S WEEKLY CAP
  --
  -- The existing G365 salary curve was calibrated to a
  -- $65,000 cap.  Treat every dollar-based pricing setting as
  -- a percentage of that reference cap so commissioners can
  -- choose a different league cap without making the player
  -- pool artificially cheap or expensive.
  -- =========================================================

  select
    coalesce(sl.weekly_salary_cap, 65000)::numeric
  into
    v_weekly_salary_cap
  from public.season_long_settings sl
  where sl.league_id = p_league_id
    and sl.season = p_season
  limit 1;

  if coalesce(v_weekly_salary_cap, 0) <= 0 then
    raise exception 'Season-Long weekly salary cap must be greater than zero.';
  end if;

  v_salary_scale :=
    v_weekly_salary_cap / 65000.0;

  v_min_salary :=
    greatest(50, round((v_min_salary * v_salary_scale) / 50.0) * 50);

  v_max_salary :=
    greatest(v_min_salary, round((v_max_salary * v_salary_scale) / 50.0) * 50);

  v_increment :=
    greatest(50, round((v_increment * v_salary_scale) / 50.0) * 50);

  v_max_weekly_increase :=
    greatest(v_increment, round((v_max_weekly_increase * v_salary_scale) / v_increment) * v_increment);

  v_max_weekly_decrease :=
    greatest(v_increment, round((v_max_weekly_decrease * v_salary_scale) / v_increment) * v_increment);
...
grant execute on function public.generate_season_long_weekly_salaries(uuid,integer,integer) to authenticated;
