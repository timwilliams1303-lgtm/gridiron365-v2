...
begin
  ...
  -- ---------------------------------------------------------
  -- UNAVAILABLE TEAMMATE / REPLACEMENT-ROLE CONTEXT
  -- ---------------------------------------------------------
  --
  -- OUT / IR / PUP / suspended players are excluded from the
  -- selectable salary pool. Their expected opportunity is not
  -- ignored, however: it can raise the salary of the most likely
  -- active replacement(s) on the same NFL team and position.
  --
  -- This is intentionally opportunity-based rather than a blind
  -- fixed bump. Losing a high-projection starter matters more than
  -- losing a low-usage reserve.
  -- ---------------------------------------------------------

  unavailable_team_position as (

    select
      pool.team_abbreviation,
      pool.position,

      count(*)::integer
        as unavailable_players,
  ...
  -- ---------------------------------------------------------
  -- SELECTABLE / PRICED WEEKLY PLAYER POOL
  -- ---------------------------------------------------------
  --
  -- Keep unavailable players in all_weekly_pool above so their lost
  -- projection/role can increase replacement salaries, but do not let
  -- those players themselves flow into pricing/upsert.
  -- ---------------------------------------------------------
  ...
  projection_components as (
    ...
    select
      ...
      -- -----------------------------------------------------
      -- REPLACEMENT OPPORTUNITY / BACKUP ROLE
      -- -----------------------------------------------------
      --
      -- When an unavailable teammate at the same position leaves
      -- meaningful projected volume behind, increase the value of
      -- the most likely active replacement(s).
      --
      -- The boost depends on:
      --   1. how much projected value became unavailable,
      --   2. the active player's role rank on that team/position,
      --   3. the position, because opportunity is redistributed
      --      differently at QB/RB/WR/TE/K.
      --
      -- Caps prevent one injury from producing an unrealistic
      -- salary spike.
      -- -----------------------------------------------------

      case
        when coalesce(
          unavailable.unavailable_projection,
          0
        ) <= 0
        then 1.00::numeric

        when pool.position = 'QB'
          and active_role.active_role_rank = 1
        then
          1.00
          +
          least(
        ...
        when pool.position = 'RB'
          and active_role.active_role_rank = 2
        then
          1.00
          +
          least(
        ...
        when pool.position = 'WR'
          and active_role.active_role_rank in (1, 2)
        then
          1.00
          +
          least(
        ...
        when pool.position = 'WR'
          and active_role.active_role_rank = 3
        then
          1.00
          +
          least(
    ...
    left join active_team_position active_role
      on active_role.player_id =
         pool.player_id

    left join unavailable_team_position unavailable
      on unavailable.team_abbreviation =
         pool.team_abbreviation

     and unavailable.position =
         pool.position
  ...
  adjusted as (
    ...
    select
      ...
      greatest(
        ...
        *
        pc.replacement_opportunity_multiplier
  ...
  select
    ...
    jsonb_build_object(
      ...
      'algorithm',
        'gridiron365_dfs_injury_pool_v7',
